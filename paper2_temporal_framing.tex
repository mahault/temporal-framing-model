...
\begin{itemize}
    \item $d^2F < 0$ while $dF < 0$: \textbf{Hope}---model fit is improving at an accelerating rate.
    \item $d^2F > 0$ while $dF < 0$: \textbf{Happiness}---model fit is stabilising into a good state.
    \item $d^2F > 0$ while $dF > 0$: \textbf{Fear}---model fit is worsening at an accelerating rate.
    \item $d^2F < 0$ while $dF > 0$: \textbf{Unhappiness}---model fit is stabilising into a bad state.
    \item Sign changes in $dF$: \textbf{Relief} (positive to negative) or \textbf{Disappointment} (negative to positive).
\end{itemize}
This taxonomy derives entirely from the dynamics of a single backward-looking quantity.
...
\paragraph{Backward (VFE-generated).} Emotions grounded in the evaluation of past model fit: \emph{regret} (VFE higher than counterfactual alternative), \emph{guilt} (VFE elevated due to own action), \emph{shame} (VFE elevated due to social model violation), \emph{pride} (VFE lower than expected), \emph{gratitude} (VFE reduction attributed to another agent).

\paragraph{Present ($-dF/dt$ or RPE).} Emotions grounded in the rate of change at the current moment: \emph{joy} (rapid VFE decrease), \emph{anger} (VFE increase attributed to external agent), \emph{disgust} (VFE increase in interoceptive modality), \emph{surprise} (large absolute $\Delta F$).

\paragraph{Forward (EFE-generated).} Emotions grounded in the evaluation of expected future performance: \emph{hope} (low EFE under preferred policy), \emph{fear} (high EFE across all policies), \emph{anxiety} (high EFE with high ambiguity), \emph{anticipation} (low EFE with low ambiguity).

\paragraph{Transitional (VFE vs prior EFE).} Emotions that arise from comparing current VFE with previously computed EFE: \emph{relief} (VFE lower than EFE had predicted), \emph{disappointment} (VFE higher than EFE had predicted), \emph{satisfaction} (VFE confirms low EFE prediction).
...
\subsection{Channel 1: $v_{\text{model}}$ --- The Backward Signal}\label{sec:v_model}
...
\subsection{Channel 2: $v_{\text{reward}}$ --- The Present Signal}\label{sec:v_reward}
...
\subsection{Channel 3: $v_{\text{action}}$ --- The Forward Signal}\label{sec:v_action}
...
When channels agree, the affective signal is unambiguous: genuine satisfaction (all positive) or genuine distress (all negative). When they disagree, the resulting states are informationally rich:
\begin{itemize}
    \item \textbf{Productive failure} ($v_{\text{model}} < 0$, $v_{\text{action}} > 0$): the current observation was bad, but the agent found a better plan. Learning from mistakes.
    \item \textbf{Unsettling success} ($v_{\text{reward}} > 0$, $v_{\text{action}} < 0$): the outcome was good, but the planning landscape worsened. Suspicion that good fortune is unsustainable.
    \item \textbf{Dawning anxiety} ($v_{\text{model}} \approx 0$, $v_{\text{reward}} \approx 0$, $v_{\text{action}} < 0$): nothing happened, but plans are deteriorating. The computational signature of anticipatory dread.
\end{itemize}
...
Each action defines a distinct transition structure $\mathbf{B}_a = \mathbf{B}_v^{(a)} \otimes \mathbf{B}_e^{(a)} \otimes \mathbf{B}_f^{(a)}$. The frame transition matrices $\mathbf{B}_f^{(a)}$ function as \emph{temporal precision operators}---they redistribute probability across temporal frames, determining which horizon the agent attends to:

\paragraph{RECALL.} $\mathbf{B}_f^{\textsc{recall}}$: 70\% self-transition to \textsc{past}. Allocates precision to past-generated internal states. Autobiographical memory retrieval as ``attending to the past.'' Simultaneously, $\mathbf{B}_v^{\textsc{recall}}$ pulls the valence state toward the prior $\mathbf{D}$ with strength $\alpha = \sigma(\pi_{\text{pos}} - \theta)$, re-accessing the agent's self-model.

\paragraph{ENGAGE.} $\mathbf{B}_f^{\textsc{engage}}$: 75\% self-transition to \textsc{present}. Maintains precision on current sensory input. Standard perception--action coupling.

\paragraph{FUTURATE.} $\mathbf{B}_f^{\textsc{futurate}}$: 90\% self-transition to \textsc{future}. Allocates precision to anticipated future states. The high absorptivity reflects ``narrative inertia''---once engaged in prospective simulation, the agent tends to persist. Simultaneously, $\mathbf{B}_v^{\textsc{futurate}}$ pulls valence toward the solution state $K{-}1$, imagining the best outcome.

\paragraph{REST.} $\mathbf{B}_f^{\textsc{rest}}$: 50--60\% to \textsc{present}. Releases frame commitment, allowing the temporal orientation to reset.

\subsection{Counterfactual Simulation as Decoupled Inference}\label{sec:decoupled}

Standard inference updates beliefs about the current state given current observations. Temporal framing extends this by performing inference about \emph{counterfactual} states---states not currently generating observations.
...
This is ``mentalisation'' in the active inference sense: inference running offline, decoupled from current sensory evidence. The connection to default mode network activation is suggestive: the DMN is maximally active during both autobiographical memory retrieval and prospective simulation \citep{friston2018deep}, consistent with these being two instances of the same counterfactual inference mechanism.
...
Each action has different risk and ambiguity because of its different $\mathbf{B}$ matrices. The agent's \emph{context}---current VFE, current beliefs, current energy---determines which temporal orientation is EFE-favoured:
\begin{equation}
    G(a) = \sum_m \left[\text{risk}_m(a) + \text{ambiguity}_m(a)\right]
\end{equation}

\begin{itemize}
    \item \textbf{High VFE $\to$ FUTURATE wins.} When beliefs are far from preferred, FUTURATE has the largest risk reduction in the valence modality because $\mathbf{B}_v^{\textsc{futurate}}$ pulls most strongly toward the preferred (solution) state.
    \item \textbf{Stable beliefs + high $\pi_{\text{pos}}$ $\to$ RECALL wins.} When the agent's model fits well but positive self-beliefs enable effective recall, RECALL maintains affect via the prior pull with minimal energy cost.
    \item \textbf{Low energy $\to$ REST wins.} When energy is depleted, REST has the lowest energy-modality risk because $\mathbf{B}_e^{\textsc{rest}}$ restores energy.
\end{itemize}

The standard softmax policy selection $\pi(a) \propto \exp(-\gamma \cdot G(a))$ then naturally routes the agent toward the appropriate temporal frame. No temporal routing heuristic is needed; the structure is already in the generative model.
...
Temporal framing policies modify \emph{which part of this loop dominates}:
\begin{itemize}
    \item RECALL amplifies the VFE$\to v_{\text{model}}$ path (backward evaluation).
    \item ENGAGE amplifies the observation$\to v_{\text{reward}}$ path (present evaluation).
    \item FUTURATE amplifies the EFE$\to v_{\text{action}}$ path (forward evaluation).
\end{itemize}
...
The energy cost of RECALL is minimal. Mentalisation---running the generative model offline on past states---is computationally cheap relative to active environmental engagement. This gives RECALL a natural advantage in the EFE decomposition: low valence risk (via prior pull) combined with low energy risk.
...
FUTURATE's energy cost creates a natural brake via EFE. After 1--2 steps of FUTURATE, the predicted interoceptive observation enters the ``depleted'' range, which increases the energy-modality risk term. The softmax shifts away from FUTURATE, producing short, reactive planning bursts rather than sustained commitment.

Simulation evidence (Figure~\ref{fig:framing_dynamics}c) confirms that FUTURATE runs are consistently shorter than RECALL runs: mean FUTURATE run length $\approx 1.3$ timesteps versus RECALL $\approx 1.7$ timesteps. The contrast is mechanistically transparent: RECALL has zero energy cost (no brake), while FUTURATE's energy cost provides an inherent self-limiting mechanism. FUTURATE is therefore a reactive, bursty planning mode---not a sustained strategic commitment.
...
The emotion--action--inference loop (\S\ref{sec:loop}) admits two dynamical regimes. Under \emph{self-regulation}, the loop is homeostatic: negative valence $\to$ faster learning $\to$ better model $\to$ positive valence \citep{joffily2013emotional}. This is the healthy default---prediction errors are informative signals that improve the model.
...
These perturbations interact through the EFE decomposition:
\begin{enumerate}
    \item \textbf{Blurred $\mathbf{A}_{\text{int}}$}: Impaired interoception ($\omega_e = 0.5$) reduces the energy-modality risk term that would normally deter FUTURATE selection. The brake is disabled.
    \item \textbf{High $\mathbf{C}$}: Elevated reward sensitivity ($c_{\text{scale}} = 2.0$) amplifies the valence-modality risk reduction for FUTURATE. The accelerator is enhanced.
    \item \textbf{FUTURATE enters the policy mix}: Under the standard softmax $\pi(a) \propto \exp(-\gamma \cdot G(a))$, FUTURATE is naturally favoured when the brake is disabled and the accelerator is enhanced.
\end{enumerate}

Once FUTURATE enters the policy mix, its 90\% absorbing frame transition creates narrative inertia---the agent tends to persist in future-oriented framing. The volatile environment then keeps violating the agent's precise future predictions, producing chronic positive $\Delta F$ (increasing VFE). This creates the self-amplifying cycle: FUTURATE $\to$ precise predictions $\to$ violation $\to$ negative $v_{\text{model}}$ $\to$ more FUTURATE (because high VFE favours FUTURATE, \S\ref{sec:futurate_trigger}).
...
The stressed agent's valence channel decomposition reveals the full temporal structure of chronic stress (Figure~\ref{fig:chronic_stress}c):
\begin{itemize}
    \item \textbf{Backward} ($v_{\text{model}}$): persistently negative. Chronic model violations from the volatile environment, amplified by FUTURATE's precise predictions.
    \item \textbf{Reward} ($v_{\text{reward}}$): oscillating around zero. The volatile environment produces unpredictable reward prediction errors; sometimes good, sometimes bad, never stable.
    \item \textbf{Action} ($v_{\text{action}}$): intermittent positive bursts. The agent keeps finding ``better'' policies under high VFE (policy improvement is easy when the current policy is poor), but the improvement is ephemeral because the environment keeps changing.
\end{itemize}

This pattern---persistent backward negativity, oscillating present, ephemeral forward positivity---is the computational signature of overmentalising: optimistic planning followed by systematic model-fit deterioration.
...
This paper has formally proposed what has been implicit in the active inference literature but never unified in a single account: the VFE/EFE distinction provides a natural temporal taxonomy of emotion. Variational free energy concerns beliefs about what \emph{has happened}---it generates backward-oriented affect (regret, guilt, pride, gratitude). Expected free energy concerns expectations about what \emph{will happen}---it generates forward-oriented affect (hope, fear, anxiety, anticipation). The rate of change at the current moment generates present-oriented affect (joy, anger, surprise). Transitional emotions (relief, disappointment) arise from comparing current VFE with prior EFE predictions.
...
Temporal framing is not specific to pathology. Healthy agents \emph{use} temporal framing for adaptive regulation: RECALL stabilises affect (creating a floor under valence), FUTURATE provides reactive planning (responding to high prediction error), and the natural energy cost of FUTURATE provides a self-limiting brake. Pathology arises not from temporal framing itself but from the failure of its balancing mechanisms: when the brake is disabled (impaired interoception), when the accelerator is enhanced (reward hypersensitivity), or when the stabiliser fails (impaired positive-belief precision).
...
\citet{hesp2021deeply} distinguished two timescales in the affect hierarchy: M4 (emotion), operating within-trial as fast precision dynamics, and M5 (mood), operating cross-trial as slow valence state inference. Temporal framing operates at the \emph{interface} between these timescales:
\begin{itemize}
    \item Individual frame selections are M4 phenomena: fast, within-trial decisions about temporal orientation, driven by current EFE.
    \item Sustained frame biases are M5 phenomena: slow, cross-trial tendencies toward particular temporal orientations, reflecting learned or dispositional patterns.
\end{itemize}
