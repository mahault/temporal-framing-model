...
### 6.6 Temporal Aiming of Affective Channels

The three-channel valence architecture provides a direct readout of the agent's *temporal orientation*: the relative magnitude of each channel reveals whether affect is predominantly backward-looking (model valence), present-oriented (reward valence), or forward-looking (action valence). We decompose the temporal orientation at each timestep as the normalised absolute channel contribution:

$$\phi_{\text{dir}}(t) = \frac{|v_{\text{dir}}(t)|}{|v_{\text{model}}(t)| + |v_{\text{reward}}(t)| + |v_{\text{action}}(t)| + \epsilon}$$

and define the *backward--forward conflict rate* as the fraction of timesteps where $v_{\text{model}}$ and $v_{\text{action}}$ have opposite signs --- measuring the degree to which the agent's past evaluation and future planning are pulling in different affective directions.

**Clinical phenotypes (Figure 8).** The three phenotypes show qualitatively distinct temporal orientation profiles. The **healthy** agent is predominantly present-oriented, with balanced backward and forward contributions and the lowest temporal conflict (30%) --- consistent with adaptive engagement grounded in both past experience and future planning. The **depressive** agent is dominated by the backward channel despite its compressed magnitude, reflecting rumination: the agent's affect is driven primarily by model-fit deterioration ($v_{\text{model}} < 0$, rising free energy) while both the present channel (anhedonia compresses $v_{\text{reward}}$) and the forward channel (policy stagnation suppresses $v_{\text{action}}$) contribute minimally. The depressive agent shows elevated backward--forward conflict (41%), indicating that when the forward channel does activate, it opposes the backward signal --- the agent's past-oriented and future-oriented assessments systematically disagree, a computational signature of hopelessness.
...
**Figure 8. Temporal aiming of affective channels.** Three rows per phenotype: (top) smoothed channel traces --- backward $v_{\text{model}}$ in blue, present $v_{\text{reward}}$ in green, forward $v_{\text{action}}$ in orange, composite in dashed black; (middle) normalised temporal orientation showing the relative contribution of each channel over time; (bottom) mean absolute channel magnitude with backward--forward conflict rate (fraction of timesteps where $v_{\text{model}}$ and $v_{\text{action}}$ have opposite signs). The healthy agent is present-oriented with balanced contributions and the lowest conflict (30%). The depressive agent is backward-dominated with compressed magnitudes (41% conflict), reflecting rumination and temporal dissonance. The manic agent shows strong present-channel dominance driven by reward hypersensitivity, with the highest conflict (47%) from rapid policy switching.

**Figure 9. Temporal orientation across emotion profiles.** Left: stacked horizontal bars showing the mean temporal orientation fraction (backward/present/forward) for each of ten emotion profiles, sorted by backward fraction. Right: backward--forward conflict rate per emotion. Backward-oriented emotions (depressed, bored, sad) cluster at the bottom with the highest conflict rates, while forward/present-oriented emotions (angry, excited) appear at the top with the lowest conflict. Fearful shows a distinctive pattern of high forward contribution combined with high temporal conflict, capturing the anticipatory quality of anxiety.
