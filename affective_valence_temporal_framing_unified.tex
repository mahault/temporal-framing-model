...
own outcome, $t=10.4$, replicated on a second dataset); and a mood layer whose
diathesis-stress prediction holds in independent data, where a baseline
vulnerability trait both lowers mood and amplifies affective stress-reactivity (the
vulnerability$\times$stress interaction, $t=4.2$), the same conjunction that in
simulation collapses into a self-sustaining low-mood attractor. Throughout we separate
...
detail tends to provoke anxiety more than it yields mastery. On the broaden-and-build
...
high-precision setting, not a universal default: under low positive-belief
precision the same forward machinery turns anxious, yielding worry-type rather than
hopeful prospection, a contingency we make explicit in the model (\S\ref{sec:model}) and test directly
...
split is the sense in which affect is a readout, sitting on top of the task model
rather than replacing it. Figure~\ref{fig:factorgraph} shows the factor graph. To read it, note that the
...
state. This produces a \emph{diathesis-stress} pattern (Figure~\ref{fig:mood}). Crossing vulnerability (low $\pi_{\text{pos}}$, blunted reward
...
depression \citep{monroe1991diathesis} and the resilience of most people to stress.

The mechanism instantiates vulnerability rather than fitting it, but its central
prediction is testable in real data, and it holds. The mechanism entails that a
vulnerability trait should both lower baseline mood and \emph{amplify} affective
reactivity to stress, a vulnerability$\times$stress interaction rather than either main
effect. In the Geschwind ESM, baseline neuroticism, which the affect-dynamics fitting
never uses, does exactly this: it lowers baseline mood ($r=-0.53$, $n=128$) and steepens
each person's affective response to daily events (between-person $r=0.31$; pooled
event$\times$neuroticism interaction $t=4.2$ over $11{,}315$ beeps), reproducing the
established neuroticism$\times$stress reactivity effect \citep{geschwind2011}. What this
$20$-day, already-remitted sample cannot show, and the mechanism additionally predicts,
is the months-scale transition into \emph{sustained} low mood from the same conjunction,
preceded by a persistent below-neutral shift in momentary valence rather than in
prediction error, a test that awaits longer longitudinal data.
...
entropy. Holds across seeds (vulnerable$+$stress depressed $8/8$; others $0/8$).
$T=3000$.}
\label{fig:mood}
\end{figure}
...
Geschwind--Bringmann ESM \citep{bringmann2013,geschwind2011} & 130 (129) & momentary affect; event pleasantness; worry; baseline neuroticism & CC BY-NC & affect-dynamics; frame-to-worry \\
...
reported orientation ($r\!\approx\!0$). This is an identifiability limit, not a model
failure: felt valence is generated by the valence state, on which the frame acts
through the \emph{dynamics} rather than instantaneously, so a single affect reading
does not determine the frame; and in the data, momentary affect carries little
orientation information in the first place (reported past-orientation explains under
$5\%$ of within-person valence variance, $r=-0.23$), which caps any affect-to-frame
recovery near that value. The validated claim is therefore a frame-to-affect coupling;
recovering orientation would require an orientation-carrying observation channel, not a
better inversion of affect.
...
The framework draws a two-level picture of depressive affect and predicts a
dissociation between the levels that the data bear out. (i) A \emph{general}
